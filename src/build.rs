...
pub struct Stats {
    ...
    pub skipped_redirect: u64,
    pub skipped_lang: u64,
    pub form_of_entries: u64,
    pub unresolved_links: u64,
    pub entries: u64,
    pub forms: u64,
    pub entries_with_forms: u64,
    pub entries_with_ipa: u64,
    pub entries_with_syllables: u64,
    pub pct_with_forms: f64,
    pub pct_with_syllables: f64,
    pub top_tags: BTreeMap<String, u64>,
}
...
pub fn run(args: &Args) -> io::Result<()> {
    ...
    if (ordered.len() as u64) < args.min_entries {
        return Err(io::Error::other(format!(
            "only {} entries, expected at least {} - wrong source URL?",
            ordered.len(),
            args.min_entries
        )));
    }
    if parse_errors * 100 > lines_read {
        return Err(io::Error::other(format!(
            "{parse_errors} parse errors across {lines_read} lines - upstream schema change?"
        )));
    }
